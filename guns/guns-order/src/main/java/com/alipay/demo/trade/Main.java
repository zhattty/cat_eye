...
public class Main {
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(String orderId, Double totalPrice) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = orderId;
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                // 这里存到C:\Program Files\nginx-1.15.12\html\aliPayQRCode 尝试使用nginx的动静分离
                String filePath = String.format("D:/seats/alipayQRCode/qr-%s.png",
                    response.getOutTradeNo());
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                return "qr-"+orderId+".png";
